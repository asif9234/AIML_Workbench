    ...
    this.modelDataService.getModelList(this.loggedUser).subscribe(
      (response: any) => {
        ...
        for (var i = 0; i < this.result.length; i++) {
          if (this.result[i].original_model_name === this.modelDetailsForm.value.model_name) {
            this.isErrorAvailable = true;
            break;
          }
        }
        // api integration
        if (this.isErrorAvailable === false) {
        ...
        console.log("checkoutTrainableModelList", checkoutTrainableModelList[0].original_model_name);
