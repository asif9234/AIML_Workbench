...
export class AllUseCaseComponent implements OnInit {
  ...
  errMessage = ''
  checkoutModel: any
  constructor(private _caseStudyService: CaseStudyService, private modelDataService: ModelDataService, private router: Router, private designWorkflowService: DesignWorkflowService, private toastService: ToastrService, private spinner: SpinnerService, private formBuilder: FormBuilder) {
  }
  ...
  editPrebuiltUsecase(event): void {
    ...
    var checkoutNonTrainableModelList = [];
    ...
    for (var i = 0; i < this.preBuiltUsecases.length; i++) {
      if (this.preBuiltUsecases[i]._id == this.usecaseID) {
        ...
        var usecaseUserId = this.mytUsecases[0].user_id;
        for (var j = 0; j < this.mytUsecases.length; j++) {
          if (this.mytUsecases[j].flow) {
            for (var k = 0; k < this.mytUsecases[j].flow.length; k++) {
              if (flow[i].type !== 'tab' && flow[i].type === this.mytUsecases[j].flow[k].type || this.mytUsecases[j].flow[k].trainingTracker_id === undefined) {
                for (var l = 0; l < flow.length; l++) {
                  if (flow[l].type !== 'tab') {
                    let trainingTracker_id = flow[l].trainingTracker_id
                    this.modelDataService.getModelDetail(trainingTracker_id).subscribe(
                      (successResponse) => {
                        var model_description = successResponse.records[0].model_description;
                        var model_name = successResponse.records[0].model_name;
                        var original_model_name = successResponse.records[0].original_model_name;
                        var prediction_params = successResponse.records[0].prediction_params;
                        var trainable = successResponse.records[0].trainable
                        var training_params = successResponse.records[0].training_params;

                        checkoutNonTrainableModelList.push({
                          user_id: usecaseUserId,
